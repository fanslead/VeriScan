...
export const rulesFromRuleSet = (ruleSet?: RuleSet | null): WordRuleDraftInput[] =>
  ruleSet?.rules.length
    ? ruleSet.rules.map(({ term, type, category, weight }) => ({ term, type, category, weight }))
    : [createEmptyRule()];
...
export function createRuleSetDraft(name: string, rules: WordRuleDraftInput[]): RuleDraftValidation {
  ...
  if (rules.length === 0) errors.push('至少需要添加一条规则');
  ...
  return errors.length > 0
    ? { errors, rowErrors }
    : { value: { name: normalizedName, rules: normalizedRules }, errors: [], rowErrors: {} };
}
