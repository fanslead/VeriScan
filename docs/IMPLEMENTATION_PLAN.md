...
- 异步批次、任务租约、重试和独立幂等取消；应用级 Webhook 配置、测试、签名、可靠发布与 dead-letter；
...
当前落地：已完成关键词、常见格式和多词组合条件，包含规范化、证据 offset、非回溯正则优先、静态安全校验与执行超时；草稿支持 checksum 校验、不可变发布、复制新版本、受绑定保护的归档、应用显式绑定、请求 `policyId` 一致性和历史版本追溯。Outbox 与 Webhook 发布队列均具备 PostgreSQL `SKIP LOCKED` 租约、失败退避、幂等消费和恢复能力；Webhook 使用官方 Svix SDK 与自托管服务，异步终态负载不含原文。双人审批、按比例灰度和独立标注评测集仍属于生产治理门禁。
...
当前落地：已完成 API/管理端非 root 多阶段镜像、同产物一次性迁移入口、liveness/readiness 分离、真实 Keycloak PKCE、细粒度后台角色、认证前入口并发保护、认证后全局/应用/Key 配额、OpenTelemetry、固定版本 Svix 服务及其独立数据库、Chromium 界面检查、规则进程内基线和完整 HTTP 短压测。生产 Provider/OIDC/TLS、真实公网 Webhook 接收端、备份恢复、60 分钟长稳、SBOM/许可证和目标集群发布仍须在部署环境完成，详见 `docs/ACCEPTANCE_REPORT.md`。

## 4. 当前完成度矩阵（2026-09-02）
...
| 同步/异步审核与幂等          |       100% | `sync/async/auto`、租约、重试、独立幂等取消、查询和异步终态 Webhook 完成                                                            |
...
| 统计、Outbox、审计与可观测性 |        97% | 不可变事实、用量重建、幂等消费、Webhook 发布状态、指标/Trace/日志和后台查询完成；生产告警平台未接入                                 |
| 管理后台与用户体验           |        98% | 业务语言规则编辑、AI/Webhook 配置、应用/Key/记录/统计/审计、权限态和响应式完成；自动视觉回归矩阵待扩充                              |
| 安全、容器和本地运维         |        95% | 原文保护、摘要 pepper、非 root、迁移 Job、健康检查、限流/背压、真实 OIDC 和 Svix 隔离数据库完成；KMS、PITR、SBOM 和目标集群演练待做 |
| RAG、多模态、主动学习        |         0% | 按 V2.1 保持关闭/暂缓，不属于当前文本审核首发完成度分母                                                                             |

按首发文本审核范围加权，当前仓库完成度约 **97%**。若把生产外部依赖与上线门禁一并计入，整体约 **80%**；剩余工作不能在本机代码仓库内伪造为完成。
...
| Infrastructure | PostgreSQL 约束、迁移、Outbox、Webhook 租约、Redis 降级、协议 fixture            |
| Frontend       | 表单校验、一次性 Key/签名密钥、测试启用门禁、错误恢复、路由权限、空/loading 状态 |
| E2E            | 创建应用 → 创建 Key → 异步审核/取消 → Webhook 终态 → 查询结果                    |
